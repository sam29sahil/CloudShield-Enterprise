...
class AzureNetwork:
    ...
    def network_interfaces(self):
        """Return Azure network interfaces."""
        try:
            interfaces = self.client.network_interfaces.list_all()
            ...
            for nic in interfaces:
                result.append({
                    ...
                    "resource_group": (
                        nic.id.split("/")[4]
                        if nic.id and "/resourceGroups/" in nic.id
                        else None
                    ),
                    "primary": getattr(nic, "primary", False),
                    "enable_accelerated_networking": getattr(
                        nic,
                    ...
                    "ip_configurations": [
                        {
                            "name": ip.name,
                            "private_ip": getattr(ip, "private_ip_address", None),
                            "private_ip_allocation": getattr(
                                ip,
                            ...
                            "public_ip_id": (
                                ...
                                if getattr(ip, "public_ip_address", None)
                                else None
                            ),
                            "subnet_id": (
                                ip.subnet.id
                                if getattr(ip, "subnet", None)
                                else None
                            ),
                        }
                        for ip in (getattr(nic, "ip_configurations", None) or [])
                    ],
                })
    ...
    def subnets(self):
        """Return Azure subnets."""
        try:
            subnets = []

            virtual_networks = self.client.virtual_networks.list_all()

            for vnet in virtual_networks:
                resource_group = (
                    vnet.id.split("/")[4]
                    if vnet.id and "/resourceGroups/" in vnet.id
                    else None
                )

                for subnet in (
                    getattr(vnet, "subnets", None) or []
                ):
                    subnets.append({
                        "id": subnet.id,
                        "name": subnet.name,
