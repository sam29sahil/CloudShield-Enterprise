...
class Parser:
    ...
    def parse(self, tool: str, target: str, result: Result | Mapping[str, Any] | None,
        ...
        return {
            "success": success, "tool": tool, "target": target,
            "summary": self.summary(raw, execution_time),
            "website": raw.get("website", {}), "headers": raw.get("headers", []),
            "ssl": raw.get("ssl", {}), "dns": raw.get("dns", {}), "whois": raw.get("whois", {}),
            "technology": raw.get("technology", []), "ports": raw.get("ports", []),
            "findings": self.findings(raw), "raw_output": raw, "command": raw.get("command", ""),
            "return_code": raw.get("return_code", 0), "execution_time": execution_time, "error": error,
        }
    ...
    def summary(self, result: Mapping[str, Any], execution_time: float) -> dict[str, Any]:
        success = bool(result.get("success", False))
        return {
            "status": "Completed" if success else "Failed",
            "message": "Scan completed successfully." if success else (
