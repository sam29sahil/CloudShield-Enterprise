...
class ScannerSettingsForm(FlaskForm):
    ...
    default_mode = SelectField(
        "Default Scan Mode", choices=[("basic", "Basic"), ("deep", "Universal")]
    )
    ...
    timeout = IntegerField(
        "Timeout (seconds)", validators=[Optional(), NumberRange(min=5, max=600)]
    )
...
class ReportSettingsForm(FlaskForm):
    ...
    include_summary = BooleanField("Include Executive Summary")

    include_recommendations = BooleanField("Include Recommendations")
...
class NotificationSettingsForm(FlaskForm):
    ...
    notify_scan_complete = BooleanField("Notify When Scan Completes")

    notify_scan_failed = BooleanField("Notify When Scan Fails")

    notify_critical = BooleanField("Notify On Critical Findings")
