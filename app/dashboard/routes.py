...
def home():
    ...
    scans = SecurityScan.query.filter_by(user_id=current_user.id).all()
    ...
    if scans:
        ...
        average_score = round(sum(s.score for s in scans) / len(scans), 2)

    recent_scans = (
        SecurityScan.query.filter_by(user_id=current_user.id)
