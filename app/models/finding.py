...
<<<<<<< HEAD
    ...
    scan_id = db.Column(db.Integer, db.ForeignKey("security_scans.id"), nullable=False)
    ...
    cwe = db.Column(db.String(30))

    owasp = db.Column(db.String(30))

    reference = db.Column(db.String(500))

    mitre = db.Column(db.String(100))
    ...
    asset = db.relationship("Asset", back_populates="findings")

    scan = db.relationship("SecurityScan", back_populates="findings")
...
<<<<<<< HEAD
        return f"<Finding " f"{self.title} " f"({self.severity})>"
=======
        return (
